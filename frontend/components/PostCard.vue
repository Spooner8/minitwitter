...
const created_at = computed(() =>
    getLocalDateTimeString(new Date(props.post.created_at))
);
const updated_at = ref(
    props.post.updated_at
        ? getLocalDateTimeString(new Date(props.post.updated_at))
        : null
);
    ...
    if (modifiedContent.value !== content) {
        try {
            ...
            if (response.status === 200) {
                const updated_at_date = getLocalDateTimeString(
                    new Date(response.data.updated_post[0].updated_at)
                );
